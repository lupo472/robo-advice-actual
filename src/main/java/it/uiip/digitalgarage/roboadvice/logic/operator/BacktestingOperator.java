...
public class BacktestingOperator extends AbstractOperator {
	...
	public List<PortfolioDTO> getBacktesting(BacktestingDTO request, Authentication auth) {
		...
		for (CustomStrategyEntity strategy : list) {
			...
			List<PortfolioDTO> listPerAsset = this.computePortfolioForAssetClass(assetClass, user, amountPerClass, date);
			result.addAll(listPerAsset);
		}
		return null;
	}

	private List<PortfolioDTO> computePortfolioForAssetClass(AssetClassEntity assetClass, UserEntity user, BigDecimal amount, LocalDate date) {
